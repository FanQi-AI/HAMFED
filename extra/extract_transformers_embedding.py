...
def extract(model_name, audio_files, save_dir, feature_level, layer_ids=None, gpu=None):
    ...
    # load model
    model_file='/home/extra/wav2vec2-large-960h'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu', type=int, default=4, help='index of GPU')
    parser.add_argument('--wav2vec2_large_960h', type=str, default='wav2vec2_large_960h', help='name of feature extractor')
    ...
    # analyze input
    layer_ids = [-1]
    original_path = "/data/MELD/dev_wav"
    #file_name = original_path.split("/")[-1]  # Extract the file name from the path
    #print(file_name)
    base_path = "/data/MELD/dev_wav"
    # new_path = os.path.join(base_path, file_name)  # Concatenate the base path with the file name
    # print(new_path)  # Print the new path
    audio_dir = base_path
    save_dir = "/data/MELD/dev_wav"
    ...
    # => save_dir
    dir_name = args.wav2vec2_large_960h if len(layer_ids) == 1 else f'{args.wav2vec2_large_960h}-{len(layer_ids)}'
    ...
    # extract features
    extract(args.wav2vec2_large_960h, audio_files, save_dir, args.feature_level, layer_ids=layer_ids, gpu=args.gpu)
